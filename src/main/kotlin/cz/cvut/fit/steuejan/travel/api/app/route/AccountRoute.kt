...
import cz.cvut.fit.steuejan.travel.api.app.extension.getUsername
...
fun Route.accountRoutes() {
    ...
    authenticate(JWT_AUTHENTICATION) {
        ...
        get<Account.LogoutAll> {
            respond(userAccountController.logoutAllDevices(getUsername()))
        }

        post<Account.ChangePassword> {
            val username = getUsername()
            val request = receive<ChangePasswordRequest>(ChangePasswordRequest.MISSING_PARAM)
            respond(userAccountController.changePassword(username, request.toChangePassword()))
        }
    }
}